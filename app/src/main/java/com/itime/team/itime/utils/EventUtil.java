            ...
            try {
                ...
                if (isValidEvent(object)) {
                    if (!decideWhetherMultiDays(object)) {
                        ...
                        Calendar startCal = DateUtil.getGMTDateToCalendar(DateUtil.getGMTDate(startString));
                        Calendar endCal = DateUtil.getGMTDateToCalendar(DateUtil.getGMTDate(endString));
                        ...
                        if (day > 0) {
                            ...
                            for (int y = 0; y <= day; y++) {
                                if (y == 0) {
                                    ...
                                    objectStart.put("event_ends_datetime", DateUtil.getDateStringFromCalendar(temp));
                                    ...
                                    objectEnd.put("event_starts_datetime", DateUtil.getDateStringFromCalendar(temp1));
                                    ...
                                    objectDuration.put("event_starts_datetime", DateUtil.getDateStringFromCalendar(temp));
                                    temp.set(Calendar.HOUR_OF_DAY, 23);
                                    temp.set(Calendar.MINUTE, 59);
                                    objectDuration.put("event_ends_datetime", DateUtil.getDateStringFromCalendar(temp));
...
//        cal.setTime(date);
        for (int i = 0; i < response.length(); i++) {
            try {
                JSONObject object = response.getJSONObject(i);
                Date date = DateUtil.getGMTDate(object.getString("event_starts_datetime"));
                Date enddate = DateUtil.getGMTDate(object.getString("event_ends_datetime"));
                Calendar calendar = DateUtil.getCalendarFromInteger(day, month, year);
                Calendar cal = DateUtil.getGMTDateToCalendar(date);
                Calendar endCal = DateUtil.getGMTDateToCalendar(enddate);
                int duration = calDurationDays(calendar, cal);
